...
def main():
    ...
    # 加载数据集
    data_f = "./Nerf-PyTorch-simple/66bdbc812bd0a196e194052f3f12cb2e.npz"
    data = np.load(data_f)
    ...
    init_ds = camera_coords.to(device)  # (100,100,3) 世界坐标系下的坐标
    init_o = torch.Tensor(np.array([0, 0, float(data["camera_distance"])])).to(device) # (3,)
    ...
    test_ds = torch.einsum("ij,hwj->hwi", test_R, init_ds) # (100,100,3)
